--- Render-verification suite for the .NET gRPC service archetype: each persistence variant lays
...
for _, persistence in ipairs({ "PostgreSQL", "MySQL" }) do
  local label = "dotnet-grpc[" .. persistence .. "]"
...
local none_project = prova.fixture("dotnet-grpc[None]:project", Scope.File, function(ctx)
  return archetect.render{
    source = SRC,
...
prova.group("dotnet-grpc[None]:image", { requires = { "docker" } }, function(g)
...
prova.group("dotnet-grpc[None]:ci", { requires = { "docker" }, tags = { "standards" } }, function(g)
